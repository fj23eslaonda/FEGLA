...
from Functions.FEGLA_engine import transect_processing, find_maxhorizontalflood, get_XR_and_R, find_steeper_slope
...
def process_transect(idx, scen_tran_all, F0):
    ...
    #max_x, max_z       = get_XR_and_R(flood_transect)
    #slope              = max_z/max_x

    # Initial Froude number according to slope and Runup
    #if slope > 1/100:
    #    F0 = 1
    #else: 
    #    F0 = 1.3
    ...
    try:
        ...
        while iteration < max_iterations:
            ...
            #if h0 > wall_height:
            Fr = F0 * (1 - distance_dummy / X_R)**0.5
            #else:
            ...
            if previous_error is not None:
                ...
                print(f'Error diff: {error_diff}')
                if error_diff < 0.001:  # If the error change is smaller than the threshold (centesimal)
        ...
        height_iteration = np.asarray([0])
        error_iteration  = np.asarray([0])
        all_XRmax        = np.asarray([0])
        all_XRmin        = np.asarray([0])
        R0               = 0
...
def main(params, scenarios_to_run=None, transect_to_run=None):
    ...
    for F0 in F02try:
        ...
        with open(f'{outpath}/{city}_all_results_F0_{F0}_squared.pkl', 'wb') as f:
