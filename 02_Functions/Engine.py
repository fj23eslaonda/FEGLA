...
def calculate_cumulative_distance(x_UTM, y_UTM):
    """
    Calculates the cumulative Euclidean distance between consecutive UTM coordinates.

    Parameters:
    x_UTM (array-like): Vector or list of UTM coordinates on the x-axis.
    y_UTM (array-like): Vector or list of UTM coordinates on the y-axis.

    Returns:
    numpy.ndarray: Array of cumulative Euclidean distances from the origin.
    """
    # Calculate squared differences and compute the Euclidean distance in a single step
    distances = np.sqrt(np.diff(x_UTM)**2 + np.diff(y_UTM)**2)
    
    # Insert 0 at the start and calculate cumulative sum in one step
    distance_cum = np.cumsum(np.insert(distances, 0, 0))

    # Round to the nearest integer
    distance_cum = np.round(distance_cum, 0)
...
def transect_processing(scen_tran_all, idx_batch, manning_coeff, max_threshold):
    """
    ...
    for idx in idx_batch:
        ...
        df = fill_initial_nans(df, 'height')
        df = filter_elevation(df, max_threshold)

        # Calculate flood levels, cumulative distance, and assign Manning's coefficient
        df['flood'] = df['elevation'] + df['height']
        df['cum_distance'] = calculate_cumulative_distance(df['x'].values, df['y'].values)
