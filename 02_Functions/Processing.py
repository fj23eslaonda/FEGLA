...
#-------------------------------------------------------
#-------------------------------------------------------
def create_spline_function(shoreline, kind_spline='cubic', n_point=1000, reduction_factor=30, smooth_window=20):
    """
    Creates a cubic spline interpolation function for a given shoreline curve using arc length parameterization.

    Parameters:
    - shoreline: numpy.ndarray, the shoreline coordinates as a 2D array where the first column is longitude and the second is latitude.
    - kind_spline: str, the type of spline ('linear', 'quadratic', 'cubic').
    - n_point: int, the number of points to generate in the smoothed spline.
    - reduction_factor: int, factor by which to reduce the number of original points used for interpolation.
    - smooth_window: int, the window size for the moving average filter.

    Returns:
    - spline_function_x: Spline function for x coordinates.
    - spline_function_y: Spline function for y coordinates.
    - smoothed_points: numpy.ndarray, array with the smoothed points (n_point, 2).
    """
    # Reduce the number of points
    shoreline_reduced = shoreline[::reduction_factor]

    # Separate the x and y coordinates
    x = shoreline_reduced[:, 0]
    y = shoreline_reduced[:, 1]
...
#--------------------------------------------
#--------------------------------------------
def perpendicular_line(x_equidistant, y_equidistant, spline_points, extension_length):
    '''
    Calculates and optionally plots a perpendicular line to a spline at specified equidistant points.

    Parameters:
    x_equidistant (numpy.ndarray): Array of x coordinates of equidistant points (longitude).
    y_equidistant (numpy.ndarray): Array of y coordinates of equidistant points (latitude).
    spline_points (numpy.ndarray): Array of points representing the spline [lon, lat].
    extension_length (float): Length of the line to be drawn in meters.

    Returns:
    numpy.ndarray: Array containing start and end points of perpendicular lines for each equidistant point.
    '''
    # Use average latitude for conversion to meters
    avg_lat = np.mean(spline_points[:, 1])

    # Convert lon/lat to meters
    lat_to_meters = 111000  # 111 km per degree
    lon_to_meters = 111000 * np.cos(np.radians(avg_lat))  # Adjusted by the average latitude

    x_meters = x_equidistant * lon_to_meters
    y_meters = y_equidistant * lat_to_meters

    spline_points_meters = np.array([
        [lon * lon_to_meters, lat * lat_to_meters] for lon, lat in spline_points
    ])
    
    # Use the center of the spline as a reference point
    ref_point = np.mean(spline_points_meters, axis=0)
    
    initial_points = []
    end_points = []
    
    for x_meter, y_meter in zip(x_meters, y_meters):
        # Find the nearest point on the spline
        distances = np.sqrt((spline_points_meters[:, 0] - x_meter)**2 + (spline_points_meters[:, 1] - y_meter)**2)
        nearest_index = np.argmin(distances)
        x_spline, y_spline = spline_points_meters[nearest_index]

        # Approximate the slope at the nearest point using points 10 indices before and after
        start_index = max(nearest_index - 10, 0)
        end_index = min(nearest_index + 10, len(spline_points_meters) - 1)
        
        x_prev, y_prev = spline_points_meters[start_index]
        x_next, y_next = spline_points_meters[end_index]
        slope = (y_next - y_prev) / (x_next - x_prev)
        
        # Calculate perpendicular slope
        perpendicular_slope = -1 / slope if slope != 0 else np.inf

        # Calculate the perpendicular line endpoints
        delta_x = extension_length / np.sqrt(1 + perpendicular_slope**2)
        delta_y = perpendicular_slope * delta_x
        
        if perpendicular_slope == np.inf:
            line_start = (x_meter, y_meter - extension_length / 2)
            line_end = (x_meter, y_meter + extension_length / 2)
        else:
            line_start = (x_meter, y_meter)
            line_end1 = (x_meter + delta_x, y_meter + delta_y)
            line_end2 = (x_meter - delta_x, y_meter - delta_y)
        
            # Choose the line direction based on the reference point
            distance1 = np.sqrt((line_end1[0] - ref_point[0])**2 + (line_end1[1] - ref_point[1])**2)
            distance2 = np.sqrt((line_end2[0] - ref_point[0])**2 + (line_end2[1] - ref_point[1])**2)
        ...
        initial_points.append(line_start)
        end_points.append(line_end)

    return np.array(initial_points), np.array(end_points)
...
#--------------------------------------------
#--------------------------------------------
def evaluate_transect(initial_point, end_point, extension_length, n_points, interpolate, avg_lat):
    """
    ...
    Parameters:
    initial_point (tuple): Tuple containing the x and y coordinates of the initial point in UTM (meters).
    end_point (tuple): Tuple containing the x and y coordinates of the end point in UTM (meters).
    ...
    interpolate (callable): Function that takes x and y coordinates and returns interpolated values.
    avg_lat (float): Average latitude for conversion between meters and degrees.
    ...
    """
    # Convert meters to degrees using the average latitude
    lat_to_meters = 111000  # 111 km per degree
    lon_to_meters = 111000 * np.cos(np.radians(avg_lat))  # Adjusted by the average latitude

    x_int, y_int = initial_point[0], initial_point[1]
    x_end, y_end = end_point[0], end_point[1]

    # Calculate the angle of the transect relative to the x-axis
    alpha = np.arctan2(y_end - y_int, x_end - x_int)

    # Generate a linear distribution of x-coordinates along the transect from initial point extended by 'extension_length'
    deltax_meters = np.linspace(0, extension_length, n_points) * np.cos(alpha) + initial_point[0]
    deltay_meters = np.linspace(0, extension_length, n_points) * np.sin(alpha) + initial_point[1]

    # Convert back to degrees
    deltax = deltax_meters / lon_to_meters
    deltay = deltay_meters / lat_to_meters

    # Perform interpolation using the precomputed interpolation function
    Z2 = interpolate(deltax, deltay)

    # Return the x-coordinates, y-coordinates along the transect, and interpolated values
    return deltax, deltay, deltax_meters , deltay_meters, Z2
